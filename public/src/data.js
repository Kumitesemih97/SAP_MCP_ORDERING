 ...
 */
export const COST_CENTER_BUDGETS = {
...
export function calculateDeliveryDate(days = SYSTEM_CONSTANTS.DEFAULT_DELIVERY_DAYS) {
    const date = new Date(Date.now() + days * 24 * 60 * 60 * 1000);
    return date.toLocaleDateString(SYSTEM_CONSTANTS.DATE_FORMAT);
}
export function findMaterialByKeyword(keyword) {
    ...
    for (const stem of stems) {
        const hit = MATERIALS.find(m => m.name.toLowerCase().includes(stem) ||
...
export function findVendorByName(name) {
    ...
    return VENDORS.find(vendor => vendor.name.toLowerCase().includes(lowerName) ||
        vendor.contact.toLowerCase().includes(lowerName)) || null;
}
...
export function calculateTotalWithTax(netAmount) {
