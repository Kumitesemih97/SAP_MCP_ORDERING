 ...
 */
import { OrderSystemError, OllamaConnectionError } from './types.js';
import { MATERIALS, VENDORS, MOCK_USER, SYSTEM_CONSTANTS, UI_TEXT, ERROR_MESSAGES, generateOrderNumber, formatCurrency, calculateTotalWithTax } from './data.js';
/**
...
export class SAPOrderingSystem {
    ...
    initializeDOMElements() {
        ...
        if (!chatMessages || !messageInput || !sendBtn) {
            throw new OrderSystemError('Required DOM elements not found', 'DOM_MISSING', { chatMessages: !!chatMessages, messageInput: !!messageInput, sendBtn: !!sendBtn });
        }
        return {
            chatMessages,
            messageInput,
    ...
    validateDOMElements() {
        ...
        const missing = required.filter(key => !this.domElements[key]);
        if (missing.length > 0) {
            throw new OrderSystemError(`Missing required DOM elements: ${missing.join(', ')}`, 'DOM_VALIDATION_FAILED', { missing });
        }
    }
    /**
     ...
     */
    async processOrderWithQwen(message) {
        try {
            ...
            if (result.success) {
                if (result.orderData) {
                    ...
                    throw new OrderSystemError(result.error || 'Processing failed without a response message', 'PROCESSING_FAILED');
                }
            }
                ...
                throw new OrderSystemError(result.error || 'Qwen processing failed', 'QWEN_PROCESSING_FAILED');
            }
        }
        catch (error) {
            this.removeTypingIndicator();
            ...
            if (error instanceof OllamaConnectionError) {
                ...
                throw new OrderSystemError('Failed to process order with Qwen', 'QWEN_REQUEST_FAILED', error);
            }
        }
    }
    /**
     ...
     */
    async handleSuccessfulOrder(orderData, originalMessage) {
     ...
     * Automatically submit order
     */
    async automaticOrderSubmission(orderData) {
     ...
     */
    displayOrderDetails(orderData, isSubmitted = false) {
     ...
     */
    createOrderForm(orderData, isSubmitted = false) {
