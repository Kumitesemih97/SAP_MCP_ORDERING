import { MATERIALS, MOCK_USER, generateOrderNumber, calculateDeliveryDate, findMaterialByKeyword, formatCurrency, } from '../data.js';
import { orderStore } from './order-store.js';
import { poService } from '../services/po-service.js';
export const createPurchaseOrderTool = {
    name: 'create_purchase_order',
    description: 'Create a SAP purchase order (ME21N). Validates stock, selects the best vendor, generates a PO number, and records the order.',
    inputSchema: {
        type: 'object',
        properties: {
            material_name: {
...
export function handleCreatePurchaseOrder(args) {
    try {
        ...
        if (!materialName)
            throw new Error('material_name is required');
        if (quantity <= 0)
...
//# sourceMappingURL=create-purchase-order.js.map