...
import { OrderSystemError } from './types.js';
import { ERROR_MESSAGES } from './data.js';
/**
...
class AppInitializer {
    ...
    async checkBrowserCompatibility() {
        ...
        if (missingFeatures.length > 0) {
            throw new OrderSystemError(`${ERROR_MESSAGES.BROWSER_INCOMPATIBLE}${missingFeatures.join(', ')}`, 'BROWSER_INCOMPATIBLE', { missingFeatures });
        }
        // Check ES6+ support
        try {
            ...
            throw new OrderSystemError('Browser does not support modern JavaScript (ES6+)', 'ES6_UNSUPPORTED');
        }
    ...
    `;
        ...
        const btnEl = document.getElementById('ollama-signin-btn');
    ...
    async initializeOrderingSystem() {
        try {
            ...
            throw new OrderSystemError('Error initializing the ordering system', 'ORDERING_SYSTEM_INIT_FAILED', error);
        }
    }
    /**
